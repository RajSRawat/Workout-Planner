...
const ExerciseLibrary = () => {
    ...
    return (
        <div className="h-full flex flex-col md:flex-row overflow-hidden">
            {/* Left Panel: List */}
            <div className="w-full md:w-1/2 lg:w-2/5 p-6 flex flex-col h-full overflow-hidden bg-black/20 md:bg-transparent z-10">
                ...
                <div className="flex-1 overflow-y-auto space-y-3 pr-2 custom-scrollbar">
                    {filteredExercises.map((ex, idx) => (
                        <motion.div
                            ...
                            className="p-4 bg-white/5 border border-white/5 rounded-xl hover:border-prime/30 transition flex justify-between items-center group cursor-pointer"
                        >
                            <div>
            ...
            {/* Right Panel: 3D Model (Hidden on small mobile, visible on larger screens) */}
            <div className="hidden md:block flex-1 p-6 h-full relative">
                <div className="h-full w-full bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-gray-800/20 via-black/50 to-black rounded-3xl border border-white/5 overflow-hidden relative">
                    <BodyModel highlightedMuscles={[selectedMuscle]} />
